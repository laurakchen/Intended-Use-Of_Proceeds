...
PARM_FORMS = EDGAR_Forms.f_424B  # or, for example, PARM_FORMS = ['8-K', '8-K/A']
...
PARM_PATH = '/Users/laurachen/Desktop/FinProject'
# PARM_PATH = r'C:\EDGAR\FORM_XX\\'

# Change the file pointer below to reflect your location for the log file
#    (directory must already exist)
PARM_LOGFILE = ('/Users/laurachen/Desktop/FinProject/' +
                'EDGAR_Download_FORM-424B_LogFile_' +
        ...
        for qtr in range(PARM_BGNQTR, PARM_ENDQTR + 1):
            ...
            if masterindex:
                for item in masterindex:
                    ...
                    # get IPO form of 424B
                    if item.form in PARM_FORMS and item.form != '4':
                        if item.cik not in ipoDict:
                            ipoDict[item.cik] = [item.filingdate, item.path, item.form]
                        else:
                            if item.filingdate < ipoDict.get(item.cik)[0]:
                                ipoDict[item.cik] = [item.filingdate, item.path, item.form]
